...
- cmd창에서 실행
```sh
# 해당 프로그램이 설치된 이미지 설치, :버전번호가 뒤에 붙으면 원하는 버전의 프로그램 설치
docker pull 응용프로그램 or 운영체제 or 개인레포지토리주소

# 설치된 이미지에 대한 정보 확인
docker images

# 실행 중인 컨테이너에 대한 정보 확인
# ps -a는 모든 상태의 프로세스 정보 확인
# -q는 실행 중인 컨테이너id만 확인
docker ps

# 해당 프로그램 삭제
docker rmi 프로그램이름 or 이미지id

# 실행 중인 컨테이너 실행 중지
# 프로세스에선 아직 남아있음 (프로세스 상태에 대한 내용을 확인하면 알 수 있음)
docker stop 컨테이너id

# 해당 컨테이너를 프로세스에서 완전 삭제
docker rm 컨테이너 id

# 실행중인 컨테이너에 접근
# 컨테이너에 설치된 운영체제에 따라 명령어 체계가 달라짐
# 나갈땐 exit
# crtl + p -> ctrl + q (프로세스를 종료하지 않고 나오기)
docker attach

# 해당 컨테이너의 명령어를 변경
docker exec 변경할명령어(-it) 컨테이너id 접근방식(bash)

# 해당 컨테이너를 이미지화하여 커밋, :1.0라는 태그를 붙혀 버전 표시
docker commit 컨테이너id 허브리포지토리주소

# 해당 이미지를 허브에 업로드
docker push 허브리포지토리주소

# 해당 컨테이너의 콘솔로그창 확인
docker logs 컨테이너id

# 해당 컨테이너의 설치된 이미지를 분석
docker inspect 컨테이너id
```

### 명령어(실행)
```sh
# 해당 프로그램 실행, 접속은 포트포워딩 이후에 가능
# 옵션은 run 명령어 바로 뒤에 붙혀줌
# 이미지에 없는 프로그램 실행시 설치도 동시 진행
docker run 프로그램이름 or 이미지id
```
```sh
# 다양한 옵션들
...
# 해당 포트번호(8080:80)로 포트포워딩
-p 8080:80

# 직접 컨테이너의 이름을 설정
--name 컨테이너이름

# 실행하려는 컨테이너와 링크된 컨테이너를 같이 실행
--link 링크할컨테이너이름

# 3개의 명령어가 결합된 형태로 d는 데몬 실행
# i는 인터랙션으로 입력가능
# t는 터미널모드로 실행, 보통 이렇게 쓰임
# bash라는 쉘 프로그램으로 작동하여 호스트 서버에서 bash를 통해 메세지 전송 가능
-dit
```
...
1. run -dit 후 attach로 접근
2. apt update (다운로드 가능한 프로그램 목록 업데이트)
3. apt install vim (vi에디터 설치)
4. cd home (home 폴더 접근)
5. mkdir ubuntu (ubuntu라는 이름의 폴더 생성)
6. vi hello (hello라는 이름의 파일 생성, 내용 입력은 aws repository를 참고)

### docker file(기본적인 과정)
1. mkdir dockerwork (bash가 가르키는 곳에 dockerwork라는 이름의 폴더 생성)
2. notepad를 통해 dockerfile 생성 (txt 확장자가 붙으면 제거, 확장자가 file이여야 함)
3. dockerfile 내용 (httpd 이미지를 가져와 .webapp에 있는 내용을 /usr/local/apache2/htdocs에 복붙, 이때 httpd-foreground 커맨드로 실행)
```sh
FROM httpd
COPY ./webapp /usr/local/apache2/htdocs
CMD ["httpd-foreground"]
```
4. dockerwork 폴더 안에 webapp 폴더를 생성 후, hello world를 띄우는 index.html 파일 생성 
5. cmd에서 dockerwork 폴더를 가리킴 
6. docker build -t webserver ./ (해당 폴더에서 dockerfile을 찾아 빌드, 이름은 webserver)
7. 이후 run하여 컨테이너 실행시키면 해당 포트로 접속시 index파일의 내용이 화면에 뜸

### 명령어(docker file)
- 각 프로젝트마다 dockerfile을 만들면 설치하려는 툴의 버전을 확인
```sh
# 호스트 서버(window)에서 도커를 통해 터미널로 컨테이너(ubuntu)로 접근시 해당 주소로 연결
WORKDIR /app

# /build에 있는 aws-v3-0.0.3.jar를 가져와 ./(WORKDIR로 폴더 지정시 해당 폴더(/app)로 시작)에 있는 application.jar로 복붙
# 압축해제 x
...
# jar 실행을 위해 java를 띄우고 jar파일이라 명시
# dev라는 옵션을 추가하고 application.jar에 설정
ENTRYPOINT ["java",  "-jar", "-Dspring.profiles.active=dev", "application.jar"]

# 서버포트를 8080으로 설정
# entrypoint는 실행, cmd는 옵션
CMD ["--server.port=8080"]

# 해당 리눅스 명령어를 실행
RUN 리눅스명령어(apt-get update, apt-get install -y nginx)

# nginx.conf 설정파일을 default.conf 설정파일에 복붙
COPY conf/nginx.conf /etc/nginx/conf.d/default.conf

# nginx 실행을 위해 -g옵션을 추가하고 데몬을 중지하는 명령어 설정 (백그라운드에서 nginx 실행시 바로 종료되므로 포그라운드 실행을 위해 종료)
ENTRYPOINT ["nginx", "-g", "daemon off;"]

# mysql 접속시 들어가기 위해 필요한 아이디 환경변수 설정
# MYSQL_PASSWORD, ROOT_PASSWORD, DATABASE도 설정해야 하고 각각 임의로 지어줌
ENV MYSQL_USER=임의아이디

# 서버 문자 인코딩을 utf8mb4로 설정, 자세한건 도커 허브 mysql 문서 참조
CMD ["--character-set-server=utf8mb4", "--collation-server=utf8mb4_unicode_ci"]

# unix와 window의 시스템 차이로 인한 개행 문자 오류 해결
...
```sh
# 호스트 서버에 있는 폴더를 볼륨 연결하여 컨테이너 실행
# c:/Spring/docker/ex05/mysql-volume 대신 임의로 이름 지어진 볼륨을 사용해도 저장 o
docker run -d -v c:/Spring/docker/ex05/mysql-volume:/var/lib/mysql -p 3307:3306 --name mysql-container mysql-image
```
...
- docker-compose에서 db는 bulid되는 Dockerfile을 해당 경로를 기준으로 구우라는 설정
- networks는 db랑 spring이랑 같은 네트워크 사용한다는 설정 (network는 키워드)
- server는 depend_on을 db로 설정하여 db가 실행이 안될 경우 server 실행도 막음 (db에 의존하여 실행)
- networks도 db와 동일하게 설정하여 같은 네트워크를 사용한다고 표시
